    ...
    const ShowCart = () => {
      ...
      return (cart.length > 0) ? cart.map((produto, index) => {return <>
        ...
        <div className="items-list-cart">
          <div className="cart-item"><img src={produto[0].img} width={250} height={150} /></div>
          ...
          <div className="cart-item">{subTotal(produto[0].price, amount[produto[0].id])}</div>
        </div>
      ...
      }) : <h4 style={{marginTop: '25%', textAlign: 'center', fontSize: '20pt', color: '#cecece'}}>Carrinho Vázio</h4>
    }
    ...
    return (
      ...
      <div class="container-cart">
          <header className="home-header">
              ...
              <span><Link className="link-home" to="/sanduiche">Sanduíches</Link></span>
          </header>
          ...
          <Link to="/finalizar"><button className="btn--finalizar">Finalizar</button></Link>
        </div>
      </div>
      </>
    );
